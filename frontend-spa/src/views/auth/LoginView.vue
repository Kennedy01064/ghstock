      ...
      <div class="relative group">
        ...
        <div class="relative bg-white/[0.04] backdrop-blur-3xl rounded-[2.5rem] border border-white/[0.12] overflow-hidden shadow-2xl">
          ...
          <form class="px-8 py-6 space-y-5" action="/auth/login" method="post" @submit.prevent="handleLogin">
            ...
            <div class="space-y-1">
              <label for="login-username-input" class="label-premium">Identificador Unico</label>
              <div class="relative group/input">
                ...
                <input id="login-username-input" data-testid="login-username" v-model="form.username" type="text" name="username" autocomplete="username" placeholder="Nombre de Usuario" class="input-field !pl-14 !bg-white/[0.06] !text-white" />
              </div>
            </div>

            <div class="space-y-1">
              <label for="login-password-input" class="label-premium">Clave de Seguridad</label>
              <div class="relative group/input">
                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none z-10">
                ...
                <input id="login-password-input" data-testid="login-password" v-model="form.password" :type="showPassword ? 'text' : 'password'" name="password" autocomplete="current-password" placeholder="Password Institucional" class="input-field !pl-14 !pr-14 !bg-white/[0.06] !text-white" />
            ...
            <label class="flex items-center cursor-pointer group/check pl-1">
              ...
              <div class="w-5 h-5 rounded-lg border-2 border-white/[0.15] bg-white/[0.08] flex items-center justify-center transition-all peer-checked:bg-amber peer-checked:border-amber group-hover/check:border-white/20">
...
async function handleLogin() {
  try {
    ...
    // Tell Chrome to offer saving credentials — await so it completes
    // before the form unmounts on navigation
