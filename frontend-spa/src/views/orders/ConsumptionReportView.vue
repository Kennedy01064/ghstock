<template>
  <div class="max-w-5xl mx-auto px-4 sm:px-8 py-8 space-y-8">
    ...
    <div v-if="rows.length" class="card !p-0 overflow-hidden border-slate-200">
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="row in rows" :key="row.key" class="hover:bg-slate-50 transition-colors group">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3 min-w-0">
                  <img
                    :src="row.imageUrl"
                    :alt="row.productName"
                    class="w-9 h-9 rounded-xl object-contain bg-slate-50 p-1.5 border border-slate-200 shrink-0"
                  >
                  <div class="min-w-0">
    ...
    <div v-else class="card border-dashed border-slate-200 bg-slate-50 py-20 text-center">
      <div class="w-16 h-16 bg-slate-50 rounded-3xl flex items-center justify-center mx-auto mb-5">
