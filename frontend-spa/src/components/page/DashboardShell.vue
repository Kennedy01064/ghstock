          ...
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-white/5">
              <tr v-for="row in table.rows" :key="row.id ?? row.label" class="hover:bg-white/[0.02] transition-colors">
                <td v-for="column in table.columns" :key="column.key" class="px-8 py-5 text-sm font-bold text-white">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
      ...
      >
        <div class="relative">
          ...
          <span
            v-if="link.count"
            class="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-navy-deep shadow-lg"
          >
            {{ link.count }}
          </span>
        </div>
        <p class="text-[11px] font-black text-text-muted uppercase tracking-[0.2em] group-hover:text-white transition-colors text-center">{{ link.label }}</p>
      </RouterLink>
    </div>
  </div>
</template>
