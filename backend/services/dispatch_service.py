...
class DispatchService:
    ...
    def create_purchase(self, purchase_in: schemas.dispatch.PurchaseCreate) -> models.Purchase:
        ...
        for item_in in purchase_in.items:
            if item_in.quantity <= 0:
                raise DomainConflictError("Purchase item quantity must be greater than 0")
                
            product = self.db.query(models.Product).filter(models.Product.id == item_in.product_id).first()
            if not product:
                continue

            self.db.add(models.PurchaseItem(
            ...
            InventoryService.register_purchase(
                db=self.db,
                product_id=product.id,
                quantity=item_in.quantity,
                actor_id=self.current_user.id,
