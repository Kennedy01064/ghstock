    ...
    <div class="card flex flex-col xl:flex-row gap-4 xl:items-center xl:justify-between">
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :class="selectedRole === option.value ? 'border-amber/40 bg-amber/10 text-amber' : 'border-white/10 bg-white/[0.03] text-text-muted hover:text-white'"
      ...
      <div class="flex flex-wrap gap-2">
        <button
          ...
          type="button"
          class="rounded-full border px-4 py-2 text-[10px] font-black uppercase tracking-[0.18em] transition-all"
          :class="selectedStatus === option.value ? 'border-white/20 bg-white/10 text-white' : 'border-white/10 bg-white/[0.03] text-text-muted hover:text-white'"
            ...
            <tr v-for="user in filteredUsers" :key="user.id" class="group transition-colors" :class="user.is_active === false ? 'bg-rose-500/[0.03] hover:bg-rose-500/[0.06]' : 'hover:bg-white/[0.03]'">
              ...
              <td class="px-6 py-6">
                <div v-if="user.assigned_buildings?.length" class="flex flex-wrap gap-2">
                  <span
                    ...
                    class="inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-white/5 border border-white/10 text-white/80 transition-colors"
                  >
                    {{ building.name }}
                  </span>
                </div>
              ...
              <td v-if="showActionsColumn" class="px-8 py-6 text-right">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button
                    type="button"
                    class="w-10 h-10 flex items-center justify-center rounded-xl border border-white/5 bg-white/5 transition-all"
...
.custom-scrollbar::-webkit-scrollbar-thumb:hover {
  background: rgba(242, 173, 61, 0.35);
}
</style>
